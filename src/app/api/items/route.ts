    ...
    try {
      ...
      if (accountsResponse.results && accountsResponse.results.length > 0) {
        const accountsToSave = accountsResponse.results
          .filter((account: any) => account.id) // Filter out accounts without id
          .map((account: any) => {
            ...
            return accountData as unknown as AccountRecord; // Type assertion needed because we're omitting id
          });
