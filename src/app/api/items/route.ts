...
async function handlePostItem(request: NextRequest) {
  ...
  try {
    ...
    if (accountsResponse.results && accountsResponse.results.length > 0) {
      ...
      if (accountsToSave.length === 0) {
        console.warn(`⚠️ No valid accounts to save (all accounts missing id) for item ${itemData.item_id}`);
      }

      const savedAccounts = await accountsService.upsertAccounts(
        accountsToSave
      );
      responseData.accounts = savedAccounts;

      console.log(`✅ Saved ${savedAccounts.length} accounts for item ${itemData.item_id}`);
      ...
      if (allTransactions.length > 0) {
        try {
          ...
          console.log(`✅ Saved ${savedTransactions.length} transactions for item ${itemData.item_id}`);
        } catch (saveError: any) {
        ...
        console.log(`ℹ️ No transactions to save for item ${itemData.item_id}`);
      }
      ...
      if (allBills.length > 0) {
        try {
          await creditCardBillsService.upsertBills(allBills);
          console.log(`✅ Saved ${allBills.length} credit card bills for item ${itemData.item_id}`);
        } catch (saveError) {
          console.error(`❌ Failed to save bills:`, saveError);
          responseData.warnings.push(
        ...
        console.log(`ℹ️ No credit card bills to save for item ${itemData.item_id}`);
      }
    } else {
      responseData.accounts = [];
    }
  } catch (accountError) {
    console.error(`❌ Failed to fetch/save accounts for item ${itemData.item_id}:`, accountError);
    responseData.warnings.push(
  ...
  try {
    ...
    if (
      ...
      const savedInvestments =
        await investmentsService.upsertInvestments(investmentsToSave);
        ...
        for (const investment of savedInvestments) {
          try {
            ...
            if (
              ...
              await investmentTransactionsService.upsertTransactions(
  ...
  try {
    ...
    if (loansResponse.results && loansResponse.results.length > 0) {
      ...
      const savedLoans = await loansService.upsertLoans(loansToSave);
...
export const POST = withErrorHandling(handlePostItem);
